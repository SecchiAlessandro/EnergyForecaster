...
def optimize_hyperparameters(X_train, y_train, n_trials=50):
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig(IMAGES_DIR / 'res_hyperparameter_importance.png', dpi=300)
        plt.close()
...
def plot_cv_train_test(fold_results, title="Cross-Validation Results"):
    ...
    plt.tight_layout()
    plt.savefig(IMAGES_DIR / 'res_cv_train_test.png', dpi=300)
    plt.close()
    
    print(f"Cross-validation train-test plot saved to: {IMAGES_DIR / 'res_cv_train_test.png'}")

def train_model(X_train, y_train, X_test, y_test, hyperparams=None):
    """Train XGBoost model for RES generation forecasting using cross-validation"""
    ...
    plot_cv_train_test(fold_results, title="RES Generation Forecasting Model")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train RES generation forecasting model')
    ...
    hyperparams_file = MODELS_DIR / 'res_model_hyperparams.joblib'
    ...
    print("- To re-optimize hyperparameters, use: python 08_train_res_model.py --reoptimize")
